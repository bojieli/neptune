...
def check_cif_file(file_path):
	f = any_file(file_path)
	structures = f.file_content.build_crystal_structures()
	models = f.file_content.model()
	jsons = []
	for s in structures:
		cs = structures[s].space_group().crystal_system()
		is_new = False
		if cs not in crystal_systems:
			crystal_systems[cs] = True
			is_new = True
		sg = structures[s].space_group().type().number()
		if sg not in space_groups:
			space_groups[sg] = True
			#is_new = True
		sys.stderr.write("Name: " + s + "\n")
		if not is_new:
			continue
		sys.stderr.write("\tUnit cell: " + str(structures[s].unit_cell().parameters()) + "\n")
		sys.stderr.write("\tCrystal system: " + structures[s].space_group().crystal_system() + "\n")
		sys.stderr.write("\tSpace group: " + structures[s].space_group().type().lookup_symbol() + "\n")
		...
		f = open(out_dir + '/' + s + '-' + cs + '-' + str(sg) + '-original.cif', 'w')
		f.write(str(models[s]))
		f.close()
		f = open(out_dir + '/' + s + '-' + cs + '-' + str(sg) + '-symmetry.cif', 'w')
		f.write(new_cif_block)
		f.close()
		f = open(out_dir + '/' + s + '-' + cs + '-' + str(sg) + '.json', 'w')
		f.write(json)
		f.close()
...
for root, subdirs, files in os.walk(walk_dir):
	...
	if not filename.endswith('.cif'):
		continue
	try:
		check_cif_file(os.path.join(root, filename))
	except:
		pass
